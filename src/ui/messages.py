...
def get_beginner_flow_guide() -> str:
    """A concise beginner flow guide shown on the landing page."""
    return """### 📖 快速上手指南

| 步骤 | 操作 | 说明 |
|------|------|------|
| **1** | 📁 上传数据 | 上传 Excel / CSV 问卷数据，或点击侧边栏「加载示例数据」快速体验 |
| **2** | 🔍 检查变量 | 在「变量识别」标签页确认变量类型推断和隐私评估结果 |
| **3** | ⚙️ 配置分析 | 在「分析配置」标签页设置目标变量、分组变量和解释变量 |
| **4** | 📊 查看结果 | 浏览单变量、双变量、多变量分析和可视化图表标签页 |
| **5** | 📄 生成报告 | 在「报告生成」标签页生成 HTML / DOCX 模板报告 |
| **6** | 🤖 AI 报告（可选）| 配置 API Key 后在「AI 智能分析」标签页生成 AI 报告 |

> 💡 **没有 API Key？** 也可以完成步骤 1-5 的全部统计分析功能。AI 报告需要额外配置 API Key。
>
> 📦 **想快速体验？** 点击侧边栏「加载内置示例数据」即可使用模拟数据走完完整流程。"""
